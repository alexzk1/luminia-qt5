...
bool LumHandler::startElement(const QString & /* namespaceURI */,
	...
	if(qName == "lumina"){ 
		...
		else if (format == "ubyte")	type = GL_UNSIGNED_BYTE;
		else if (format == "short")	type = GL_SHORT;
		else if (format == "ushort")	type = GL_UNSIGNED_SHORT;
		else if	(format == "int")	type = GL_INT;
...
bool LumHandler::endElement(const QString & /* namespaceURI */,
	...
	if(qName == "node" || qName == "mesh" || qName == "stream" || qName == "bone" || qName == "armature" ){
		...
		QByteArray d = QByteArray::fromBase64(content.toAscii());
...
void LumGenerator::processItem(Item *item, int depth){
	...
	if (type == "World"){
		...
		switch(co->getCompType()){
			...
			case Item_component::COLOR: comptype = "color"; break;
		...
		switch (co->getFormat()){
			case GL_BYTE:		out << " format=\"byte\" >\n"; break;
			case GL_UNSIGNED_BYTE:	out << " format=\"ubyte\" >\n"; break;
			case GL_SHORT:		out << " format=\"short\" >\n"; break;
			case GL_UNSIGNED_SHORT:	out << " format=\"ushort\" >\n"; break;
			case GL_INT:		out << " format=\"int\" >\n"; break;
			case GL_UNSIGNED_INT:	out << " format=\"uint\" >\n"; break;
			case GL_HALF_FLOAT_ARB:	out << " format=\"half\" >\n"; break;
			case GL_FLOAT:
			default:
				out << " format=\"float\" >\n";
			}
