...
void Item_mesh::genNormal(Item_component* p){
	...
	if (I_position == NULL || indexlist.size() == 0){
		QMessageBox::warning ( 0,QString("Normal generator error") , "This generator needs 1 Vertex component and 1 or more indices" );
		return;
		}
	...
	for ( int k = 0; k < keyframes ;k ++){
		for (int j = 0; j < indexlist.size(); j ++){
			...
			if (ipp == 3)for (int t = 0; t < I_index->num_of_primitives * 3; t += 3){
				...
				n(0,na,k) += cx; n(1,na,k)+=cy; n(2,na,k)+=cz;
				n(0,nb,k) += cx; n(1,nb,k)+=cy; n(2,nb,k)+=cz;
				n(0,nc,k) += cx; n(1,nc,k)+=cy; n(2,nc,k)+=cz;
			...
			if (ipp == 4)for (int q = 0; q < I_index->num_of_primitives * 4; q += 4){
				...
				n(0,nb,k) += cx; n(1,nb,k)+=cy; n(2,nb,k)+=cz;
				n(0,nc,k) += cx; n(1,nc,k)+=cy; n(2,nc,k)+=cz;
				n(0,nd,k) += cx; n(1,nd,k)+=cy; n(2,nd,k)+=cz;
				}
			}
		}
	//normalize
