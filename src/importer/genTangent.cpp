...
void Item_mesh::genTangent(Item_component* _position, Item_component* _normal, Item_component* _texcoord){
	...
	if (I_position==NULL)for ( int i = 0; i < complist.size(); i ++){
		if(complist.at(i)->getCompType()== Item_component::VERTEX){
			I_position = complist.at(i);
			break;
			}
		}

	Item_component* I_normal = _normal;
	if (I_normal==NULL)for ( int i = 0; i < complist.size(); i ++){
		if(complist.at(i)->getCompType()== Item_component::VECTOR){
			I_normal = complist.at(i);
			break;
			}
		}

	Item_component* I_texcoord = _texcoord;
	if (I_texcoord==NULL)for ( int i = 0; i < complist.size(); i ++){
		if(complist.at(i)->getCompType()== Item_component::UVCOORDS){
			I_texcoord = complist.at(i);
			break;
			}
		}
	...
	if (I_position == NULL || I_normal == NULL || I_texcoord== NULL || indexlist.size() == 0){
		QMessageBox::warning ( 0,QString("Tangent generator error") , "This generator needs 1 Vertex component 1 Vector component, 1 UVcoord and 1 or more indices" );
		return;
		}
	...
	int keyframes = I_position->getKeyFrames();
	//int keyframes_tex = I_texcoord->getKeyFrames();
	if (keyframes != I_normal->getKeyFrames() ){
		QMessageBox::warning ( 0,QString("Tangent generator error") , "Vertex (Position) and Vector (Normal) don't have the same number of keyframes");
		return;
		}
	...
	for ( int k = 0; k < keyframes ;k ++){
		...
		for (int j = 0; j < indexlist.size(); j ++){
			Item_index *I_index = indexlist.at(j);
			...
			if (ipp == 3)for (int t = 0; t < I_index->num_of_primitives * 3; t += 3){
				...
				*/
				...
				tan(0,ia,k) += cx; tan(1,ia,k)+=cy; tan(2,ia,k)+=cz;
				tan(0,ib,k) += cx; tan(1,ib,k)+=cy; tan(2,ib,k)+=cz;
				tan(0,ic,k) += cx; tan(1,ic,k)+=cy; tan(2,ic,k)+=cz;

				}

			if (ipp == 4)for (int q = 0; q < I_index->num_of_primitives * 4; q += 4){
				...
				tan(0,ib,k) += cx; tan(1,ib,k)+=cy; tan(2,ib,k)+=cz;
				tan(0,ic,k) += cx; tan(1,ic,k)+=cy; tan(2,ic,k)+=cz;
				tan(0,id,k) += cx; tan(1,id,k)+=cy; tan(2,id,k)+=cz;
				}
			}
		}
	for ( int k = 0; k < keyframes ;k ++){
