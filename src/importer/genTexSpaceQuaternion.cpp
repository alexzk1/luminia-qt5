...
void Item_mesh::genTexSpaceQuaternion(Item_component* _I_normal, Item_component* _I_tangent)
{
    ...
    if (!I_normal || !I_tangent)
    {
        ...
        QComboBox cn(nullptr);
        grid.addWidget(&cn, 0, 1);

        QLabel lt(tr("Tangent:"));
        grid.addWidget(&lt, 1, 0);
        QComboBox ct(nullptr);
        ...
        for (auto i : complist)
        {
            if (i->getCompType() != Item_component::VECTOR)
                continue;
            cn.addItem(i->objectName());
            ct.addItem(i->objectName());
        }
    ...
    unsigned tkey = I_tangent->getKeyFrames();
    unsigned nkey = I_normal->getKeyFrames();
    auto keyframes = std::min(tkey,  nkey);
    ...
    {
        for (uint32_t i = 0 ; i < num_of_vertices * keyframes ; ++i)
        {
            ...
            mat[1] = t(1, i, k);
            mat[2] = t(2, i, k);
            S = std::sqrt(mat[0] * mat[0] + mat[1] * mat[1] + mat[2] * mat[2]);
            mat[0] /= S;
            mat[1] /= S;
            mat[2] /= S;
            ...
            S = std::sqrt(mat[6] * mat[6] + mat[7] * mat[7] + mat[8] * mat[8]);
            mat[6] /= S;
            mat[7] /= S;
            mat[8] /= S;
            ...
            S = 1.0 / std::sqrt(Q[0] * Q[0] + Q[1] * Q[1] + Q[2] * Q[2] + Q[3] * Q[3]);
    ...
    auto *interlink = new QList<int>[num_of_vertices];
    ...
    auto *b = new bool[num_of_vertices];
    for (unsigned k = 0 ; k < keyframes; k++)
    {
        for (uint32_t i = 0 ; i < num_of_vertices; i++ )
            b[i] = false;

        for (uint32_t i = 0 ; i < num_of_vertices; i++ )  // loop that starts the flipping
        {
