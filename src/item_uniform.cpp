...
QString Item_uniform::statusText() const
{
    ...
    {
        case GL_INT:
            return QString(tr("%5; %1 %2 Int%4")).arg(k).arg(size).arg(dim).arg(Item::statusText());
        default:
...
void Item_uniform::Uniform(QObject* _shader, QString var)
{
    if (glwrapper_shader* shader = dynamic_cast<glwrapper_shader*>(_shader))
    {
        ...
        if (format == GL_FLOAT)
        {
            ...
            {
                case 16:
                    glUniformMatrix4fv(loc,  size, GL_FALSE, buf.f);
                    return;
                case 9:
                    glUniformMatrix3fv(loc,  size, GL_FALSE, buf.f);
                    return;
                case 4:
                    glUniform4fvARB(loc, size, buf.f);
                    return;
                case 3:
                    glUniform3fvARB(loc, size, buf.f);
                    return;
                case 2:
                    glUniform2fvARB(loc, size, buf.f);
                    return;
                case 1:
                    glUniform1fvARB(loc, size, buf.f);
                    return;
            }
        }
        else
            if (format == GL_INT)
            {
                ...
                {
                    case 4:
                        glUniform4ivARB(loc, size, (GLint*) buf.i);
                        return;
                    case 3:
                        glUniform3ivARB(loc, size, (GLint*) buf.i);
                        return;
                    case 2:
                        glUniform2ivARB(loc, size, (GLint*) buf.i);
                        return;
                    case 1:
                        glUniform1ivARB(loc, size, (GLint*) buf.i);
                        return;
                }
            }
            else
...
void Item_uniform::UniformInterpolator( QObject* _shader, const QString& var, float mix, int keyframe1, int keyframe2)
{
    if (glwrapper_shader* shader = dynamic_cast<glwrapper_shader*>(_shader))
    {
        ...
        if (format == GL_FLOAT)
        {
            ...
            {
                case 16:
                    glUniformMatrix4fv(loc,  size, GL_FALSE, fbuf);
                    return;
                case 9:
                    glUniformMatrix3fv(loc,  size, GL_FALSE, fbuf);
                    return;
                case 4:
                    glUniform4fvARB(loc, size, fbuf);
                    return;
                case 3:
                    glUniform3fvARB(loc, size, fbuf);
                    return;
                case 2:
                    glUniform2fvARB(loc, size, fbuf);
                    return;
                case 1:
                    glUniform1fvARB(loc, size, fbuf);
                    return;
            }
